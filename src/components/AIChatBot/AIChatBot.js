...
const EmptyChat = () => {
    const tags = [`I am ${name}`, "Meet My Bot", "Discover Me", "Portfolio Chatbot", "Interview My Bot", "Ask My AI"];
    ...
    return (
        <div className="empty-chat-container">
            ...
            <div className="empty-chat-prompt">Meet my bot <span>{name}</span> — your smart guide to me.</div>
        </div>
    );
};
...
const AIChat = ({ setShowChat, stack, setStack }) => {
    ...
    const fetchResponse = async (userInput) => {
        try {
            ...
            const initialPrompt = `
                ...
                Treat youself as ${greeting.username} and give answer as someone is interviewing you.
    ...
    return (
        <div className={`chat-window glassmorphism ${isEnlarged ? 'enlarged' : ''}`}>
            <div className="chat-header">
                ...
                <div className="header-controls">
                    <button onClick={toggleEnlarge} className="control-button">
                        {isEnlarged ? <TbArrowsMinimize size={18} /> : <TbArrowsMaximize size={18} />}
                    </button>
                    <button onClick={() => setShowChat(false)} className="control-button close-button">
                        <FaWindowMinimize />
                    </button>
                </div>
            </div>
                ...
                {stack.length === 0 ? (
                    ...
                    stack.map((item, index) => (
                        <div key={index} className="message-container">
                            ...
                            {item.response && <ReactMarkdown className="message-ai">{item.response || "Unale"}</ReactMarkdown>}
                        </div>
                    ))
                )}
...
const AIChatBot = () => {
    ...
    return (
        <>
            ...
            {showChat && <AIChat stack={stack} setStack={setStack} setShowChat={setShowChat} />}
        </>
    );
};
